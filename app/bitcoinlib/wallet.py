# Copyright (C) The python-app.bitcoinlib developers
#
# This file is part of python-app.bitcoinlib.
#
# It is subject to the license terms in the LICENSE file found in the top-level
# directory of this distribution.
#
# No part of python-app.bitcoinlib, including this file, may be copied, modified,
...
import app.bitcoinlib
import app.bitcoinlib.base58
import app.bitcoinlib.bech32
import app.bitcoinlib.core
import app.bitcoinlib.core.key
import app.bitcoinlib.core.script as script
...
class CBitcoinAddress(object):
    ...
    def __new__(cls, s):
        ...
        except app.bitcoinlib.bech32.Bech32Error:
            pass

        try:
            return CBase58BitcoinAddress(s)
        except app.bitcoinlib.base58.Base58Error:
            pass
...
class CBech32BitcoinAddress(app.bitcoinlib.bech32.CBech32Data, CBitcoinAddress):
...
class CBase58BitcoinAddress(app.bitcoinlib.base58.CBase58Data, CBitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion):
        ...
        if nVersion == app.bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR']:
            self.__class__ = P2SHBitcoinAddress

        elif nVersion == app.bitcoinlib.params.BASE58_PREFIXES['PUBKEY_ADDR']:
...
class P2SHBitcoinAddress(CBase58BitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion=None):
        if nVersion is None:
            nVersion = app.bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR']

        elif nVersion != app.bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR']:
    ...
    def from_scriptPubKey(cls, scriptPubKey):
        ...
        if scriptPubKey.is_p2sh():
            return cls.from_bytes(scriptPubKey[2:22], app.bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR'])
    ...
    def to_scriptPubKey(self):
        ...
        assert self.nVersion == app.bitcoinlib.params.BASE58_PREFIXES['SCRIPT_ADDR']
...
class P2PKHBitcoinAddress(CBase58BitcoinAddress):
    ...
    def from_bytes(cls, data, nVersion=None):
        if nVersion is None:
            nVersion = app.bitcoinlib.params.BASE58_PREFIXES['PUBKEY_ADDR']
    ...
    def from_pubkey(cls, pubkey, accept_invalid=False):
        ...
        if not accept_invalid:
            if not isinstance(pubkey, app.bitcoinlib.core.key.CPubKey):
                pubkey = app.bitcoinlib.core.key.CPubKey(pubkey)
            if not pubkey.is_fullyvalid:
                raise CBitcoinAddressError('invalid pubkey')

        pubkey_hash = app.bitcoinlib.core.Hash160(pubkey)
    ...
    def from_scriptPubKey(cls, scriptPubKey, accept_non_canonical_pushdata=True, accept_bare_checksig=True):
        ...
        if accept_non_canonical_pushdata:
            ...
            except app.bitcoinlib.core.script.CScriptInvalidError:
                raise CBitcoinAddressError('not a P2PKH scriptPubKey: script is invalid')

        if scriptPubKey.is_witness_v0_keyhash():
            return cls.from_bytes(scriptPubKey[2:22], app.bitcoinlib.params.BASE58_PREFIXES['PUBKEY_ADDR'])
        elif scriptPubKey.is_witness_v0_nested_keyhash():
            return cls.from_bytes(scriptPubKey[3:23], app.bitcoinlib.params.BASE58_PREFIXES['PUBKEY_ADDR'])
    ...
    def to_scriptPubKey(self, nested=False):
        """Convert an address to a scriptPubKey"""
        assert self.nVersion == app.bitcoinlib.params.BASE58_PREFIXES['PUBKEY_ADDR']
...
class CKey(object):
    ...
    def __init__(self, secret, compressed=True):
        self._cec_key = app.bitcoinlib.core.key.CECKey()
        self._cec_key.set_secretbytes(secret)
        self._cec_key.set_compressed(compressed)

        self.pub = app.bitcoinlib.core.key.CPubKey(self._cec_key.get_pubkey(), self._cec_key)
...
class CBitcoinSecretError(app.bitcoinlib.base58.Base58Error):
    pass

class CBitcoinSecret(app.bitcoinlib.base58.CBase58Data, CKey):
    ...
    def from_secret_bytes(cls, secret, compressed=True):
        ...
        self = cls.from_bytes(secret + (b'\x01' if compressed else b''),
                              app.bitcoinlib.params.BASE58_PREFIXES['SECRET_KEY'])
        self.__init__(None)
        return self

    def __init__(self, s):
        if self.nVersion != app.bitcoinlib.params.BASE58_PREFIXES['SECRET_KEY']:
            raise CBitcoinSecretError('Not a base58-encoded secret key: got nVersion=%d; expected nVersion=%d' % \
                                      (self.nVersion, app.bitcoinlib.params.BASE58_PREFIXES['SECRET_KEY']))
