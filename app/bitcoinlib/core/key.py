...
# Copyright (C) 2012-2015 The python-app.bitcoinlib developers
#
...
import app.bitcoinlib.signature

import app.bitcoinlib.core.script
...
class CECKey:
    ...
    def sign(self, hash): # pylint: disable=redefined-builtin
        ...
        assert 1 == result
        if app.bitcoinlib.core.script.IsLowDERSignature(mb_sig.raw[:sig_size0.value]):
            return mb_sig.raw[:sig_size0.value]
        else:
    ...
    def sign_compact(self, hash): # pylint: disable=redefined-builtin
        ...
        sig = app.bitcoinlib.signature.DERSignature.deserialize(sig)
